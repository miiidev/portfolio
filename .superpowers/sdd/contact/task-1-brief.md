...
```powershell
Select-String -Path src\components\ContactFooter.tsx -Pattern "rounded-full bg-accent/10 text-accent"
```

Expected: 0 matches — the old text pills used `inline-flex items-center min-h-11 px-4 rounded-full bg-accent/10 text-accent text-sm font-bold`; the new buttons use `h-11 w-11` circles. If the old pill class string still appears, the replacement failed.
