### Task 13: Projects restyle â€” tab headers on cards
...
The `ProjectsSection` section wrapper keeps `{...fadeRightConfig}`; note the section element already receives variants via `fadeRightConfig` â€” if `SectionHeading` sits inside a `motion.div` without variants, wrap it as shown above (if the existing heading is a direct child, keep the same wrapper structure).
